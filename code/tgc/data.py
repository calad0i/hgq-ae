...
def get_data_and_mask(fname, seed=42, split=(0.7, 0.1, 0.2), mask_thres=0.01):
    with h5.File(fname, 'r') as f:
    ...
    X_train = [X_train[:, :, :3], X_train[:, :, 3:5], X_train[:, :, 5:]]
    X_val = [X_val[:, :, :3], X_val[:, :, 3:5], X_val[:, :, 5:]]
    X_test = [X_test[:, :, :3], X_test[:, :, 3:5], X_test[:, :, 5:]]
